...
GartenPlaner.prototype.addTask = async function() {
    try {
        ...
        if (window.logger) {
            ...
            window.logger.error('Failed to add task', 'app', { error: error.message }, error);
        }
        if (window.rateLimiter) {
            window.rateLimiter.rollbackRequest('taskCreate');
        }
...
GartenPlaner.prototype.toggleArchiveView = function() {
    ...
    if (this.bulkMode) { this.toggleBulkMode(); }
    this.renderTasks();
    this.updateStatistics();
};
...
GartenPlaner.prototype.archiveTask = async function(id) {
    try {
        ...
        if (window.errorBoundary) {
            window.errorBoundary.handleError({ type: 'runtime', message: 'Failed to archive task: ' + error.message, error: error, function: 'archiveTask', context: { taskId: id }, timestamp: new Date().toISOString() });
        }
...
GartenPlaner.prototype.unarchiveTask = async function(id) {
    try {
        ...
        if (!task) { throw new Error(`Archived task with id ${id} not found`); }
        ...
        if (window.errorBoundary) {
            window.errorBoundary.handleError({ type: 'runtime', message: 'Failed to unarchive task: ' + error.message, error: error, function: 'unarchiveTask', context: { taskId: id }, timestamp: new Date().toISOString() });
        }
...
GartenPlaner.prototype.deleteArchivedTask = async function(id) {
    try {
        ...
        if (confirm('M\u00f6chten Sie diese archivierte Aufgabe endg\u00fcltig l\u00f6schen?')) {
            if (this.useAPI) { await TaskAPI.deleteArchivedTask(id); }
            this.archivedTasks = this.archivedTasks.filter(t => String(t.id) !== String(id));
            await this.saveArchivedTasks();
            this.renderTasks();
        ...
        if (window.errorBoundary) {
            window.errorBoundary.handleError({ type: 'runtime', message: 'Failed to delete archived task: ' + error.message, error: error, function: 'deleteArchivedTask', context: { taskId: id }, timestamp: new Date().toISOString() });
        }
...
GartenPlaner.prototype.toggleTaskStatus = async function(id) {
    try {
        ...
        if (this.useAPI) { await TaskAPI.updateTask(id, { status: newStatus }); }
        task.status = newStatus;
        task.completedAt = task.status === 'completed' ? new Date().toISOString() : null;
        this.addHistoryEntry(task, task.status === 'completed' ? 'completed' : 'reopened', { from: oldStatus, to: task.status });
        await this.saveTasks();
        this.renderTasks();
        this.updateStatistics();
        ...
        if (window.errorBoundary) {
            window.errorBoundary.handleError({ type: 'runtime', message: 'Failed to toggle task status: ' + error.message, error: error, function: 'toggleTaskStatus', context: { taskId: id }, timestamp: new Date().toISOString() });
        }
        this.showNotification('\u274c Fehler beim \u00c4ndern des Aufgabenstatus. Bitte versuchen Sie es erneut.', 'error');
    }
};

GartenPlaner.prototype.calculateNextDue = function(recurrence, fromDate = new Date()) {
        ...
        if (task.recurrence && task.recurrence !== 'none' && task.nextDue) {
            ...
            if (now >= nextDueDate) {
                const newTask = {
                    ...
                    title: task.title, employee: task.employee, location: task.location,
                    description: task.description, status: 'pending',
                    createdAt: new Date().toISOString(), recurrence: task.recurrence,
...
GartenPlaner.prototype.saveTasks = async function() {
    try {
        ...
        if (window.errorBoundary) {
            window.errorBoundary.handleError({ type: 'storage', message: 'Failed to save tasks: ' + error.message, error: error, function: 'saveTasks', context: { taskCount: this.tasks.length }, timestamp: new Date().toISOString() });
        }
...
GartenPlaner.prototype.saveArchivedTasks = async function() {
    try {
        ...
        if (window.errorBoundary) {
            window.errorBoundary.handleError({ type: 'storage', message: 'Failed to save archived tasks: ' + error.message, error: error, function: 'saveArchivedTasks', context: { archivedTaskCount: this.archivedTasks.length }, timestamp: new Date().toISOString() });
        }
...
GartenPlaner.prototype.clearAllData = async function() {
    try {
        const confirmed1 = await this.showConfirm({ title: '\u26a0\ufe0f WARNUNG', icon: '\u26a0\ufe0f', message: 'M\u00f6chten Sie wirklich ALLE Daten l\u00f6schen?\n\nDiese Aktion kann nicht r\u00fcckg\u00e4ngig gemacht werden!', confirmText: 'Weiter', cancelText: 'Abbrechen', danger: true });
        if (confirmed1) {
            const confirmed2 = await this.showConfirm({ title: '\u26a0\ufe0f LETZTE WARNUNG', icon: '\ud83d\udea8', message: 'Sind Sie sich absolut sicher?\n\nAlle Aufgaben werden unwiderruflich gel\u00f6scht!', confirmText: 'Ja, alles l\u00f6schen', cancelText: 'Abbrechen', danger: true });
            if (confirmed2) {
        ...
        if (window.errorBoundary) {
            window.errorBoundary.handleError({ type: 'runtime', message: 'Failed to clear all data: ' + error.message, error: error, function: 'clearAllData', context: {}, timestamp: new Date().toISOString() });
        }
        this.showNotification('\u274c Fehler beim L\u00f6schen der Daten. Bitte versuchen Sie es erneut.', 'error');
    }
};

GartenPlaner.prototype.addHistoryEntry = function(task, action, details = {}) {
    if (!task.history) { task.history = []; }
    task.history.push({ timestamp: new Date().toISOString(), action: action, details: details });
};
    ...
    this.tasks.forEach(task => {
        if (task.history && task.history.length > 0) {
            task.history.forEach(entry => {
                allHistory.push({ ...entry, taskId: task.id, taskTitle: task.title, taskEmployee: task.employee });
            });
        }
    });
...
GartenPlaner.prototype.bulkCompleteTasksAction = function() {
    if (this.selectedTasks.size === 0) { this.showNotification('\u26a0\ufe0f Keine Aufgaben ausgew\u00e4hlt'); return; }
    this.selectedTasks.forEach(taskId => {
        const task = this.tasks.find(t => t.id === taskId);
        if (task && task.status !== 'completed') { task.status = 'completed'; task.completedAt = new Date().toISOString(); }
    });
    this.saveTasks(); this.selectedTasks.clear(); this.renderTasks(); this.updateStatistics();
...
GartenPlaner.prototype.bulkUncompleteTasksAction = function() {
    ...
    this.selectedTasks.forEach(taskId => {
        const task = this.tasks.find(t => t.id === taskId);
        if (task && task.status === 'completed') { task.status = 'pending'; task.completedAt = null; }
    });
    this.saveTasks(); this.selectedTasks.clear(); this.renderTasks(); this.updateStatistics();
...
GartenPlaner.prototype.bulkDeleteTasksAction = async function() {
    ...
    const confirmed = await this.showConfirm({ title: 'Aufgaben l\u00f6schen', icon: '\ud83d\uddd1\ufe0f', message: `M\u00f6chten Sie wirklich ${count} Aufgabe(n) l\u00f6schen?`, confirmText: 'L\u00f6schen', cancelText: 'Abbrechen', danger: true });
    if (confirmed) {
        this.tasks = this.tasks.filter(task => !this.selectedTasks.has(task.id));
        this.selectedTasks.clear();
        this.saveTasks(); this.renderTasks(); this.updateStatistics(); this.updateEmployeeFilter(); this.updateLocationFilter();
        this.showNotification(`\ud83d\uddd1\ufe0f ${count} Aufgabe(n) gel\u00f6scht`);
    }
};

GartenPlaner.prototype.bulkArchiveTasksAction = function() {
    if (this.selectedTasks.size === 0) { alert('Bitte w\u00e4hlen Sie mindestens eine Aufgabe aus.'); return; }
    const count = this.selectedTasks.size;
    if (confirm(`M\u00f6chten Sie wirklich ${count} Aufgabe(n) archivieren?`)) {
        ...
        tasksToArchive.forEach(task => { task.archivedAt = new Date().toISOString(); this.archivedTasks.push(task); });
        this.tasks = this.tasks.filter(task => !this.selectedTasks.has(task.id));
        this.selectedTasks.clear();
        this.saveTasks(); this.saveArchivedTasks(); this.renderTasks(); this.updateStatistics(); this.updateEmployeeFilter(); this.updateLocationFilter();
