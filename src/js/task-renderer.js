...
GartenPlaner.prototype.renderTasks = function() {
    ...
    filteredTasks.sort((a, b) => {
        ...
        var orderA = a.sortOrder !== undefined ? a.sortOrder : this.tasks.indexOf(a);
        var orderB = b.sortOrder !== undefined ? b.sortOrder : this.tasks.indexOf(b);
...
GartenPlaner.prototype.createTaskCard = function(task) {
    ...
    var safeDescription = task.description ? Security.escapeHtml(task.description) : '';
...
GartenPlaner.prototype.renderCalendar = function() {
...
GartenPlaner.prototype.updateLocationFilter = function() {
    ...
    var locations = [...new Set(this.tasks.map(task => task.location).filter(loc => loc))].sort();
    var currentValue = filterSelect.value;
...
GartenPlaner.prototype.updateStatistics = function() {
    if (this.showArchive) {
        ...
        var archivedCompleted = this.archivedTasks.filter(t => t.status === 'completed').length;
        var archivedPending = this.archivedTasks.filter(t => t.status === 'pending').length;
...
GartenPlaner.prototype.updateProgressBars = function(pending, completed, total, employees) {
...
GartenPlaner.prototype.updateAdditionalStats = function() {
    ...
    if (statLocations) {
        var locations = new Set(this.tasks.filter(t => t.location).map(t => t.location));
...
GartenPlaner.prototype.getActionInfo = function(action) {
    var actions = {
...
GartenPlaner.prototype.getHistoryDetailsHTML = function(entry) {
...
GartenPlaner.prototype.getTimeAgo = function(date) {
...
GartenPlaner.prototype.getRecurrenceBadge = function(recurrence) {
    var labels = {
        'daily': 'Täglich',
        'weekly': 'Wöchentlich',
        'monthly': 'Monatlich'
...
GartenPlaner.prototype.getSubtaskProgress = function(task) {
    if (!task.subtasks || task.subtasks.length === 0) {
...
GartenPlaner.prototype.showNotification = function(message) {
...
GartenPlaner.prototype.announce = function(message) {
