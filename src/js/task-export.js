...
GartenPlaner.prototype.exportData = function() {
    try {
        ...
        if (window.errorBoundary) {
            window.errorBoundary.handleError({ type: 'runtime', message: 'Failed to export data: ' + error.message, error: error, function: 'exportData', context: { taskCount: this.tasks.length }, timestamp: new Date().toISOString() });
        }
...
GartenPlaner.prototype.exportPDF = async function() {
    try {
        if (typeof window.jspdf === 'undefined') {
            await this.showConfirm({ title: 'Fehler', icon: '\u274c', message: 'PDF-Export ist nicht verf\u00fcgbar. Bitte laden Sie die Seite neu.', confirmText: 'OK', cancelText: '', danger: false });
            return;
        }
        ...
        if (tasksToExport.length === 0) {
            await this.showConfirm({ title: 'Keine Aufgaben', icon: '\u2139\ufe0f', message: 'Keine Aufgaben zum Exportieren vorhanden.', confirmText: 'OK', cancelText: '', danger: false });
            return;
        }
        doc.setFontSize(20);
        ...
        tasksToExport.forEach((task, index) => {
            ...
            yPosition += 6;
            if (task.location) { doc.text(`Standort: ${task.location}`, margin + 5, yPosition); yPosition += 6; }
        ...
        if (window.errorBoundary) {
            window.errorBoundary.handleError({ type: 'runtime', message: 'Failed to export PDF: ' + error.message, error: error, function: 'exportPDF', context: {}, timestamp: new Date().toISOString() });
        }
    ...
    try {
        ...
        reader.onload = async (e) => {
            try {
                ...
                if (window.errorBoundary) {
                    window.errorBoundary.handleError({ type: 'runtime', message: 'Failed to parse import file: ' + error.message, error: error, function: 'importData', context: { fileName: file.name }, timestamp: new Date().toISOString() });
                }
                await this.showConfirm({ title: 'Fehler', icon: '\u274c', message: 'Fehler beim Importieren: Ung\u00fcltige Datei', confirmText: 'OK', cancelText: '', danger: true });
            }
        };
        ...
        if (window.errorBoundary) {
            window.errorBoundary.handleError({ type: 'runtime', message: 'Failed to import data: ' + error.message, error: error, function: 'importData', context: {}, timestamp: new Date().toISOString() });
        }
